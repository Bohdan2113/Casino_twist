...
loginFormDiv.querySelector('form').addEventListener('submit', async e => {
  ...
  try {
    ...
    if (data.accessToken) {
      ...
      // Ти можеш додати редірект тут, наприклад:
...
registerFormDiv.querySelector('form').addEventListener('submit', async e => {
  ...
  const username = registerFormDiv.querySelector('#register-username').value.trim();
  const password = registerFormDiv.querySelector('#register-password').value.trim();
  ...
  try {
    ...
    // Після успішної реєстрації можна переключити на логін форму автоматично
    setTimeout(() => {
      registerFormDiv.style.display = 'none';
      loginFormDiv.style.display = 'block';
      clearMessages();
